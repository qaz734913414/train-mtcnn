...
class NegativeMiningOperator_OnlyLandmark106(mx.operator.CustomOp):
    def __init__(self, landmark_ohem=config.LANDMARK_OHEM, landmark_ohem_ratio=config.LANDMARK_OHEM_RATIO):
        super(NegativeMiningOperator_OnlyLandmark106, self).__init__()
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self.landmark_ohem:
            ...
            for i in range(num):
                #print L1_error[i]
                if L1_error[i] < config.landmark_L1_thresh*212:
                    landmark_keep[i] = 0
        else:
            landmark_keep += 1
...
@mx.operator.register("negativemining_onlylandmark106")
class NegativeMiningProp_OnlyLandmark106(mx.operator.CustomOpProp):
    def __init__(self):
        super(NegativeMiningProp_OnlyLandmark106, self).__init__(need_top_grad=False)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return NegativeMiningOperator_OnlyLandmark106()
