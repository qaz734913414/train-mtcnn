...
class NegativeMiningOperator_OnlyLandmark17(mx.operator.CustomOp):
    def __init__(self, landmark_ohem=config.LANDMARK_OHEM, landmark_ohem_ratio=config.LANDMARK_OHEM_RATIO):
        super(NegativeMiningOperator_OnlyLandmark17, self).__init__()
        self.landmark_ohem = landmark_ohem
        self.landmark_ohem_ratio = landmark_ohem_ratio

    def forward(self, is_train, req, in_data, out_data, aux):
        landmark_pred = in_data[0].asnumpy() # batchsize x 34
        landmark_target = in_data[1].asnumpy() # batchsize x 34
        ...
        if self.landmark_ohem:
            ...
            for i in range(num):
                #print L1_error[i]
                if L1_error[i] < config.landmark_L1_thresh*34:
                    landmark_keep[i] = 0
        else:
            landmark_keep += 1
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        landmark_grad = np.repeat(landmark_keep, 34, axis=1)
...
@mx.operator.register("negativemining_onlylandmark17")
class NegativeMiningProp_OnlyLandmark17(mx.operator.CustomOpProp):
    def __init__(self):
        super(NegativeMiningProp_OnlyLandmark17, self).__init__(need_top_grad=False)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return NegativeMiningOperator_OnlyLandmark17()
