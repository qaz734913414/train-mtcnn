...
from train_onlylandmark106_offline import train_net
from core.symbol import L106_Net80
...
def train_L_net(image_set, root_path, dataset_path, prefix, ctx,
    ...
    imdb = IMDB("mtcnn", image_set, root_path, dataset_path, 'train', -1)
    gt_imdb = imdb.get_annotations()
    sym = L106_Net80('train')

    train_net(sym, prefix, ctx, pretrained, epoch, begin_epoch, end_epoch, gt_imdb, batch_size, thread_num,
              80, False, False, True, frequent, not resume, lr, lr_epoch)

def parse_args():
    parser = argparse.ArgumentParser(description='Train L106_Net80',
    ...
    parser.add_argument('--prefix', dest='prefix', help='new model prefix',
                        default='model/lnet106_80', type=str)
